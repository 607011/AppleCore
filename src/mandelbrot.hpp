...
struct mandelbrot_calculator_perturbative
{
...
template <typename FloatType> struct mandelbrot_calculator
{
    ...
    iteration_count_t calculate(FloatType const& x0, FloatType const& y0, const iteration_count_t max_iterations)
    {
        FloatType x = 0;
        FloatType y = 0;
        FloatType x2 = 0;
    ...
    void calculate_mandelbrot_row_range(thread_param const& p)
    {
        for (int y = p.start_row; y < p.end_row; ++y)
        {
            ...
            {
                FloatType const& pixel_real = p.real_start + p.scale_factor * x;
                FloatType const& pixel_imag = p.imag_start + p.scale_factor * y;
